...
local writeHeadPositionChange = function()
	...
	if file_object == nil then
		...
		if initPass or yaw ~= last_yaw or maximumTimeoutReached then
			changedValues[#changedValues + 1] = string.format(
				", [expr] v360 yaw 'lerp(%.3f,%.3f,(T-%.3f)/%.3f)'",
				last_yaw,
				yaw,
				lasttimePos,
				movementDuration
...
local startNewLogSession = function()
	if file_object == nil then
		...
		writeHeadPositionChange()
		mp.osd_message(
			string.format("Started Motion Record %s_3dViewHistory_%s.txt", videofilename, fileobjectNumber),
			0.5
		)
	else
		mp.osd_message(
			string.format("Stopped Motion Record %s_3dViewHistory_%s.txt", videofilename, fileobjectNumber),
			2.5
		)
		writeHeadPositionChange()
